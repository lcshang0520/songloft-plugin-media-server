...
const handleStream: Handler = async (_req, query) => {
  ...
  const directUrl = (song as any).url || ''
  if (directUrl) {
    return { statusCode: 302, headers: { Location: directUrl }, body: '' }
  }

  const filePath = getSongFilePath(song)
  if (filePath) {
    try {
      const fileUrl = await songloft.plugin.getFileUrl(filePath)
      if (fileUrl) {
        return { statusCode: 302, headers: { Location: fileUrl }, body: '' }
      }
    } catch (e) {
      songloft.logger.warn('Failed to build song file URL, falling back to serveFile: ' + String(e))
    }
  }
...
function getSongFilePath(s: any): string {
  return String(s.filePath || s.file_path || '')
}

function songToSubsonic(s: any) {
  const fp = getSongFilePath(s)
  const urlPath = String(s.url || '').split('?')[0].split('#')[0]
  const pathForSuffix = fp || urlPath
  const rawSuffix = pathForSuffix.includes('.') ? pathForSuffix.split('.').pop() || '' : ''
  const suffix = (rawSuffix || 'mp3').toLowerCase()
